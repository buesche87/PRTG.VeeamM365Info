<#
    ...
    .EXAMPLE
        powershell.exe -NoLogo -NonInteractive -ExecutionPolicy Bypass -File "C:\Script\VeeamM365Info-XML.ps1"
...
# PRTG parameters
$ExpWarning = 30 # Warninglevel in days for license or SSL Certificate expiry
$ExpError   = 14 # Errorlevel in days for license or SSL Certificate expiry
...
function Set-XMLContent {
    ...
    $result+=   "  <CustomUnit>Days</CustomUnit>" + $nl
    $result+=   "  <LimitMinWarning>$ExpWarning</LimitMinWarning>" + $nl
    $result+=   "  <LimitWarningMsg>Zertifikat läuft aus</LimitWarningMsg>" + $nl
    $result+=   "  <LimitMinError>$ExpError</LimitMinError>" + $nl
    $result+=   "  <LimitErrorMsg>Zertifikat läuft aus</LimitErrorMsg>" + $nl
    $result+=   "  <LimitMode>1</LimitMode>" + $nl
    $result+=   "  <showChart>1</showChart>" + $nl
    $result+=   "  <showTable>1</showTable>" + $nl
    $result+=   "</result>" + $nl
...
$SSLCertificate = Get-VBOSecuritySettings
...
# Get SSL Certificate
$VeeamInfos.SSLExpDays = ($SSLCertificate.CertificateExpirationDate - (Get-Date)).Days
